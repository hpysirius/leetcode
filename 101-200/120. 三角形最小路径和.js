...
var minimumTotal = function (nums) {
    ...
    for (let i = 1; i < n; i++)
        for (let j = 0; j <= i; j++) {
            f[i & 1][j] = Number.MAX_VALUE;
            if (j > 0) f[i & 1][j] = Math.min(f[i & 1][j], f[i - 1 & 1][j - 1] + nums[i][j]);
            if (j < i) f[i & 1][j] = Math.min(f[i & 1][j], f[i - 1 & 1][j] + nums[i][j]);
        }
    let ans = Number.MAX_VALUE;
    for (let i = 0; i < n; i++) ans = Math.min(ans, f[n - 1 & 1][i]);
    return ans;
};
